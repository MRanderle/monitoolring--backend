...
`POST` é usado tanto para criar um resource "normal" (`POST /ferramentas`)
quanto para registrar um **evento de transição de estado** como
sub-resource de ação: `POST /emprestimos/{id}/devolucao`,
`POST /emprestimos/{id}/perda`. Essa segunda forma existe porque o PRD
...
`configuracoes/prazo-devolucao` (FR-8). Substitui o valor de configuração
...
`PATCH` de status para `inativa` (FR-4) — não existe rota `DELETE
/ferramentas/{id}`.
...
|---|---|---|---|---|
| `POST` | `/auth/registro` | Membro cria a própria conta | Público | `201` |
...
| `GET` | `/ferramentas` | Lista catálogo com disponibilidade em tempo real (FR-5) | Membro/Staff | `200` |
| `POST` | `/ferramentas` | Cadastra nova Ferramenta (FR-3) | Staff | `201` |
| `GET` | `/ferramentas/{id}` | Detalha uma Ferramenta | Membro/Staff | `200` |
| `PATCH` | `/ferramentas/{id}` | Edita dados ou inativa (FR-4) | Staff | `200` |
| `GET` | `/membros/{id}` | Consulta dados e bloqueio do próprio Membro (FR-7) ou de qualquer Membro (Staff, FR-6) | Membro (próprio)/Staff | `200` |
| `GET` | `/membros/{id}/emprestimos` | Histórico de empréstimos do Membro | Membro (próprio)/Staff | `200` |
| `GET` | `/emprestimos` | Lista empréstimos (Staff filtra por status/membro) | Staff | `200` |
| `POST` | `/emprestimos` | Staff confirma Retirada, com Checklist embutido (FR-9–11); rejeitado com `409` se o Membro tiver empréstimo em atraso (FR-12) | Staff | `201` |
| `GET` | `/emprestimos/{id}` | Detalha um Empréstimo | Membro (próprio)/Staff | `200` |
| `GET` | `/emprestimos/{id}/checklists` | Consulta checklist(s) de retirada e devolução lado a lado (FR-14) | Membro (próprio)/Staff | `200` |
| `POST` | `/emprestimos/{id}/devolucao` | Staff confirma Devolução com Checklist; aceita flag de divergência (FR-13–16) | Staff | `200` |
| `POST` | `/emprestimos/{id}/perda` | Staff registra Perda, fechando o Empréstimo sem Checklist de devolução (FR-19) | Staff | `200` |
| `GET` | `/configuracoes/prazo-devolucao` | Consulta o prazo padrão configurado | Staff | `200` |
| `PUT` | `/configuracoes/prazo-devolucao` | Define/atualiza o prazo padrão (FR-8) | Staff | `200` |

## 3. Convenção de status codes

| Código | Quando usar |
|---|---|
| `200 OK` | Leitura bem-sucedida, ou ação/transição que retorna a representação atualizada do resource (ex.: `POST /emprestimos/{id}/devolucao`) |
| `201 Created` | Criação de um novo resource identificável (ex.: `POST /ferramentas`, `POST /emprestimos`) |
